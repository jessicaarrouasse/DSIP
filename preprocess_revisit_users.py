...
from utils import save_dataframe, load_pickle
from create_user_history_db import user_history_feature_extraction
...
def compute_scaling(selected_features, df, scaler=None):
...
def preprocess_train(df):
    ...
    df = pd.read_csv("./data/clean_full_train.csv")
    # 2. Split Data - random split
    ...
    save_pickle(train_stats, "train_stats_new.pkl")
    ...
    user_history_feat = user_history_feature_extraction(train_data)
    user_history_feat.to_csv("data/user_history_db.csv", index=False)
    # Merge train_data with user_history_feat on user_id
    train_data = train_data.merge(
        user_history_feat, 
        on='user_id', 
        how='left'  
    )
    # Identify the columns from user_history_feat that need NaN replacement
    # user_history_columns = user_history_feat.columns
    # Fill NaN only in user history columns with -1
    # train_data[user_history_columns] = train_data[user_history_columns].fillna(-1)
    ...
    # merge with history from db (train set)
...
def preprocess_predict(df, train_stats=None, scaler=None):
    df = clean_data_predict_mode(df)
    test_data = df.copy()
    # 2. Feature Extraction
    # 2.1 Load Statistics on the Training Set + Scaler
    if not train_stats:
        train_stats = load_pickle("./data/train_stats.pkl")
    # if not scaler:
    ...
    test_data = feature_extraction(test_data, train_stats)
    user_history_feat = pd.read_csv("data/user_history_db.csv")
    # merge with history from db (train set)
    test_data = test_data.merge(
        user_history_feat, 
        on='user_id', 
