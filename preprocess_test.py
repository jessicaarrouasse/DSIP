...
def main(test_csv_path):
   # 1. Clean Data
   df = parse(test_csv_path)
   df = clean_data(df)
   # 2. Split Data - no need
   ...
   X_test, _ = feature_extraction(test_data, selected_features, train_stats, scaler=scaler)
   # Save the features and labels s csv files
   save_dataframe(X_test, "data/X_test_1st.csv")


if __name__ == '__main__':
   parser = argparse.ArgumentParser()
   parser.add_argument("--test-csv-path", type=str)
   args = parser.parse_args()
   main(args.test_csv_path)